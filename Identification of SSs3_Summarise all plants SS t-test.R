...
for (i in SS_summaries) {
  ...
  SS_per_plant$plant <- rep(plant, times=length(SS_per_plant$med_MRPM))
...
for (i in plant_types){
  ...
  summary_statsR <- Plant.indiv_ed %>% 
    filter(source == "Root") %>% 
    group_by(SS) %>% 
    summarise(Root_meanMRPM = mean(Adjusted_abund))
  summary_statsS <- Plant.indiv_ed %>% 
    filter(source == "Soil") %>% 
    group_by(SS) %>% 
    summarise(Soil_meanMRPM = mean(Adjusted_abund))
...
for (i in plant_types){
  ...
  Logfold_dat <- Logfold_dat %>% 
    group_by(SS,source) %>% 
    summarise(avg_MRPM=mean(Adjusted_abund))
  Logfold_dat_ed <- pivot_wider(Logfold_dat, names_from = source, values_from = avg_MRPM)
...
MRPM_select <- Combined_logFold
MRPM_select$MRPM_abund <- ifelse((MRPM_select$Root > MRPM_select$Soil),MRPM_select$Root,MRPM_select$Soil)

#Create final figure, including the logfold data, t-test p-values and abundance of SS in the rhizosphere
Fig_labels <-  MRPM_select$plant %>% 
    unique()
  Fig_labels <-  sort(Fig_labels, decreasing = FALSE)
  MRPM_select$plant <- factor(MRPM_select$plant, levels=Fig_labels)
Logfold_plot <- ggplot(MRPM_select, aes(x = log2, y = plant, fill=signif, size=MRPM_abund)) +
